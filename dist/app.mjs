...
import {resolveView} from './access.mjs';
...
function showView(){const route=resolveView(location.hash,!!user);let key=route.view;if(route.loginRequired){pendingRoute=route.pendingRoute;history.replaceState(null,'','#zones');if(!$('auth-dialog').open)openAuth(route.pendingRoute||'');}for(const v of ['zones','workouts','planner'])$(`${v}-view`).hidden=v!==key;document.querySelectorAll('[data-view]').forEach(a=>{const active=a.dataset.view===key;a.classList.toggle('active',active);if(active)a.setAttribute('aria-current','page');else a.removeAttribute('aria-current');});}
addEventListener('hashchange',showView);

function renderAuth(){const b=$('auth-button'),lock=$('planner-lock');b.textContent=user?`Hi, ${user.user_metadata?.display_name||user.email.split('@')[0]}`:'Log masuk';b.classList.toggle('signed-in',!!user);b.title=user?'Klik untuk log keluar':'';lock.hidden=!!user;renderMenu();showView();}
...
$('plan-form').addEventListener('submit',e=>{e.preventDefault();if(!user){openAuth('planner');return;}planStarted=true;updatePlan();if(plan){$('save-program').hidden=false;$('save-status').textContent='Semak cadangan ini, kemudian tekan Mula program.';}});
